...
export default function GeneralDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
        ...
        if (ticketsData) {
            // Construir Map una sola vez: O(n) en lugar de O(n×m) en cada loop
            const ticketsByEventId = new Map<string, typeof ticketsData>()
            const eventsById = new Map(eventsData.map(e => [e.id, e]))

            for (const t of ticketsData) {
                const list = ticketsByEventId.get(t.event_id) ?? []
                list.push(t)
                ticketsByEventId.set(t.event_id, list)
            }

            const resolvePrice = (t: { paid_price: number | null; ticket_tiers: { price: number } | null }) => {
                const p = Number(t.paid_price)
                return p > 0 ? p : Number(t.ticket_tiers?.price) || 0
            }
            ...
            if (next) {
                const nextEvtTickets = ticketsByEventId.get(next.id) ?? []
                const sold = nextEvtTickets.length
                const totalStock = (next.ticket_tiers as { total_stock: number }[] ?? [])
                    .reduce((acc, tier) => acc + (tier.total_stock || 0), 0)
            ...
            setRecentSales(ticketsData.slice(0, 5).map(t => ({
                ...
                eventTitle: eventsById.get(t.event_id)?.title || 'Evento',
            ...
            const matrix = eventsData.map(evt => {
                const evtTickets = ticketsByEventId.get(evt.id) ?? []
                const revenue = evtTickets.reduce((sum, t) => sum + resolvePrice(t), 0)
                return {
                    id: evt.id,
                    title: evt.title,
            ...
            for (const evt of eventsData) {
                ...
                const stock = (evt.ticket_tiers as { total_stock: number }[] ?? [])
                    .reduce((acc, tier) => acc + (tier.total_stock || 0), 0) || 100
