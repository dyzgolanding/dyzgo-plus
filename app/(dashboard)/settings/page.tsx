...
export default function SettingsPage() {
  ...
  const handleImageUpload = (field: 'logo_url' | 'banner_url') => async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      toast.success('Imagen subida correctamente. No olvides "Guardar Cambios".')
    } catch (error: unknown) {
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        toast.success(isCreating ? 'Productora creada correctamente ✅' : 'Configuración guardada correctamente ✅')
    } catch (error: unknown) {
        const msg = error instanceof Error ? error.message : 'Error desconocido'
  ...
  const handleDelete = async () => {
      ...
      if (confirmText !== profile.name) {
          return toast.error("El nombre no coincide. Operación cancelada.")
      }
